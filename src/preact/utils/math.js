...
const randomNumber = (maxDigits) => {
  return Math.ceil(Math.random() * (10 ** maxDigits))
}
...
const carryoverAndProduct = (multiplicant, multiplier, placeValueTenthExponent) => {
  if (Number.isNaN(multiplier) || multiplier > 10) {
  ...
  range(placeValueTenthExponent).forEach(() => {
    return productAsDigits.push(0)
  })
  return {
    carryOvers,
...
const placeValues = (powerTen) => {
  return reverse(range(powerTen).map((i) => {
    switch (i) {
      case 0:
        return 'O';
      case 1:
...
const multiplicationRow = (selectedDigit, i) => {
  const mult = `${selectedDigit} x ${i + 1} = ${selectedDigit * (i + 1)}`;
  return (
    <div key={i}>
      {mult}
    </div>
